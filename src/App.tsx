...
`;
  ...
  // ── Reset entire checklist indices & rearrange options to initial default order ──
  const triggerResetChecklist = () => {
    if (editMode) {
      // In edit mode - reset all checkboxes of ALL modes and restore options to initial order
      ...
      Object.keys(modes).forEach((m) => {
        ...
        if (DEFAULT_MODES[m]) {
          updatedModes[m] = {
            ...updatedModes[m],
            options: [...DEFAULT_MODES[m].options],
          };
        }
      });
      ...
      // Reset checkboxes and options of ONLY the selected current mode block
      const nextSelections = { ...selections, [currentMode]: [] };
      setSelections(nextSelections);
      localStorage.setItem('fm_sel_' + currentMode, JSON.stringify([]));

      if (modes[currentMode] && DEFAULT_MODES[currentMode]) {
        const updatedModes = {
          ...modes,
          [currentMode]: {
            ...modes[currentMode],
            options: [...DEFAULT_MODES[currentMode].options],
          },
        };
        setModes(updatedModes);
        localStorage.setItem('fm_modes', JSON.stringify(updatedModes));
